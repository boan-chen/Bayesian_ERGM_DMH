...
for s = 1:L % Monte Carlo repetitions
    ...
    beta(1,:) = [-3.0 0.9 0.95 -0.95];
    sig2(1) = 1;
    z0 = randn(N,1); % individual random effects
    ...
    for t = 2:T % start MCMC
        ...
        if ltnt == 1 % DMH algorithm to update latent variables
            ...
            for k = 1:K
                z1 = z0;
                if k == K
                    z1((k-1)*M+1:N) = z0((k-1)*M+1:N)+c0*randn(N-(k-1)*M,1);
                else
                    z1((k-1)*M+1:k*M) = z0((k-1)*M+1:k*M)+c0*randn(M,1);
                end
            ...
            if acc_rate0(t)<0.4 && c0>=1e-10
                c0 = c0/1.01;
            elseif acc_rate0(t)>0.6 && c0<=1.0
                c0 = c0*1.01;
            end
        ...
        if t < 500
            ...
            beta1 = mvnrnd(beta0,cov(beta(1:t-1,:))*2.38^2/nk)*0.95+mvnrnd(beta0,c1*eye(nk))*0.05;
        end
        ...
        for r = 1:R  % start to simulate auxiliary networks
            for i = 1:N
                for j = 1:N
                    if i~=j
                        pw = H(i,j)+beta1(3)*W1(j,i)+beta1(4)*W1(j,:)*W1(:,i);
                        pw = ((-1)^W1(i,j))*pw;
                        if log(rand(1)) <= pw
                            W1(i,j) = 1-W1(i,j);
                        end
        ...
        if abs(sum(sum(W1))-sum(sum(W))) > 60  % condition to reject the generated network
            ...
            pp  = ZZ0*(beta1-beta0)'+ZZ1*(beta0-beta1)'+log(mvnpdf(beta1,zeros(1,nk),100*eye(nk)))-log(mvnpdf(beta0,zeros(1,nk),100*eye(nk)));
