...
class ergm_double_metropolis_hastings:
    ...
    def auxiliary_network(self, beta, r=1000):
        H = beta[0] + beta[1] * (self.Xn - self.Xn.T) + self.ltnt * (self.Zn + self.Zn.T)
        Wn = np.double(H > 0)
    ...
    def update_latent(self, z1, sig2, beta0, Wn):
        ...
        for k in range(1, self.K + 1):
            z0 = z1.copy()
            if k == self.K:
                z1[(k - 1) * self.M:] = z0[(k - 1) * self.M:] + np.sqrt(self.c0) * np.random.randn(self.N - (k - 1) * self.M) 
            else:
                z1[(k - 1) * self.M:k * self.M] = z0[(k - 1) * self.M:k * self.M] + np.sqrt(self.c0) * np.random.randn(self.M)
            ...
            if abs(np.sum(W1)) - np.sum(W) <= 60:
                ...
                if k == self.K:
                    pp += mvnorm.logpdf(z1[(k - 1) * self.M:], np.zeros(self.N - (k - 1) * self.M),
                                       sig2 * np.identity(self.N - (k - 1) * self.M)) - mvnorm.logpdf(
                        z0[(k - 1) * self.M:], np.zeros(self.N - (k - 1) * self.M), sig2 * np.identity(self.N - (k - 1) * self.M))
                else:
                    pp += mvnorm.logpdf(z1[(k - 1) * self.M:k * self.M], np.zeros(self.M),
                                       sig2 * np.identity(self.M)) - mvnorm.logpdf(z0[(k - 1) * self.M:k * self.M],
                                                                          np.zeros(self.M),
    ...
    def update_hyperparams(self, sig2, z1):
        sig2 = (1 / chi2.rvs(self.alpha + self.N)) * ((z1 - z1.mean()) ** 2).sum() + self.kappa
        return sig2
    ...
    def propose_beta(self, beta0, beta, Wn, nk):
        ...
        if t < 500:
            ...
            beta1 = mvnorm.rvs(beta0, np.cov(beta[:t-1].T) * 2.38**2 / nk) * 0.6 + mvnorm.rvs(beta0,
                                                                                               self.c1 * np.identity(nk)) * 0.4
        ...
        if abs(np.sum(W1)) - np.sum(W) > 60:
            ...
            ZZ1 = [np.sum(W1), np.sum(np.dot(W1, (X - X.T))), np.sum(np.dot(W1, W1)) / 2, np.sum(np.dot(W1, np.dot(W1, W1))) / 3]
            ZZ0 = [np.sum(W), np.sum(np.dot(W, (X - X.T))), np.sum(np.dot(W1, W1)) / 2, np.sum(np.dot(W1, np.dot(W1, W1))) / 3]
            dzz = np.array(ZZ1) - np.array(ZZ0)
            dbeta = np.array(beta1 - beta0).T
            pp = np.dot(dzz, dbeta) + mvnorm.logpdf(beta1, np.zeros(nk), 100 * np.identity(nk)) - mvnorm.logpdf(
                beta0, np.zeros(nk), 100 * np.identity(nk))
    ...
    def estimate(self):
        ...
        for self.t in tqdm(range(1, self.T)):
            ...
            if self.ltnt == 1:
                ...
                if acc_rate0[self.t] < 0.2 and self.c0 >= 1e-10:  
                    self.c0 /= 1.01
                elif acc_rate0[self.t] > 0.3 and self.c0 <= 1.0:
                    self.c0 *= 1.01
...
#%%
estimator = ergm_double_metropolis_hastings(Wn[0], Xn[0], Zn[0], ltnt = 0, beta0 = [-3.5, 1.1, 1.1, -1.1])
